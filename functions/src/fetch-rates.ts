 ...
 * No API key required
 */
export async function fetchEuribor(
  tenor: "6m" | "12m",
  startDate: string,
  endDate: string
): Promise<RateEntry[]> {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!cols || cols.length <= Math.max(timeIdx, valueIdx)) continue;
...
export async function fetchSOFR(
  ...
  const byMonth: Record<string, { date: string; rate: number }> = {};
 ...
 */
export async function fetchBoEBaseRate(
  startDate: string,
  endDate: string
): Promise<RateEntry[]> {
  ...
  const monthlyEntries: RateEntry[] = [];
 ...
 */
export async function fetchCanadaRate(
  startDate: string,
  endDate: string
): Promise<RateEntry[]> {
  ...
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Bank of Canada API error: ${res.status}`);
