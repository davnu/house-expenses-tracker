...
describe('validateAttachmentFiles', () => {
  it('accepts a supported file under the size limit', () => {
    const { accepted, rejection } = validateAttachmentFiles(
      ...
      { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: 0 },
    )
  ...
  it('enforces household storage quota with running accumulator', () => {
    ...
    const { accepted, rejection } = validateAttachmentFiles(
      ...
      { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: MAX_HOUSEHOLD_STORAGE - remaining },
    )
    expect(accepted).toHaveLength(1)
  ...
  it('counts stagedFiles bytes toward the quota (preserves legacy FileDropZone math)', () => {
    ...
    const { accepted, rejection } = validateAttachmentFiles(
      ...
      {
        maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: MAX_HOUSEHOLD_STORAGE - 500, // 500 remaining
  ...
  it('dedupes against stagedFiles by name+size', () => {
    ...
    const { accepted } = validateAttachmentFiles(
      ...
      { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: 0, stagedFiles: staged },
    )
    expect(accepted).toHaveLength(0)
  })
  ...
  it('does not dedupe when opt-out is set (document flow)', () => {
    ...
    const { accepted } = validateAttachmentFiles(
      [makeFile('receipt.pdf', 'application/pdf', 5000)],
      { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: 0, stagedFiles: staged, dedupe: false },
    )
    expect(accepted).toHaveLength(1)
  })
  ...
  it('accepts files that fill household quota to EXACTLY MAX_HOUSEHOLD_STORAGE', () => {
    const { accepted, rejection } = validateAttachmentFiles(
      ...
      { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: MAX_HOUSEHOLD_STORAGE - 500 },
    )
    expect(accepted).toHaveLength(1)
    expect(rejection).toBeNull()
  })
  ...
  it('accepts upload when household is already at quota minus zero', () => {
    ...
    const { accepted } = validateAttachmentFiles(
      ...
      { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: MAX_HOUSEHOLD_STORAGE },
    )
    expect(accepted).toHaveLength(1)
  })
  ...
  it('throws loudly when maxHouseholdBytes is undefined (fails-safe for JS callers)', () => {
    // Previously the validator defaulted to MAX_HOUSEHOLD_STORAGE — which caused
    // the 50 MB Pro-user regression. Removing the default means a TypeScript
    // bypass (e.g. untyped JS import, `as any` cast) would silently accept any
    // size since comparisons against `undefined` return false. Runtime guard
    // turns that silent fail-open into a loud fail-loud.
    expect(() =>
      validateAttachmentFiles(
        [makeFile('a.pdf', 'application/pdf', 1000)],
        // deliberate cast to simulate a caller that skipped the required param
        { householdStorageUsed: 0 } as unknown as { maxHouseholdBytes: number; householdStorageUsed: number },
      ),
    ).toThrow(TypeError)
  })

  it('throws when maxHouseholdBytes is negative (defensive)', () => {
    expect(() =>
      validateAttachmentFiles(
        [makeFile('a.pdf', 'application/pdf', 1000)],
        { householdStorageUsed: 0, maxHouseholdBytes: -1 },
      ),
    ).toThrow(TypeError)
  })

  it('throws when maxHouseholdBytes is NaN / Infinity (defensive)', () => {
    expect(() =>
      validateAttachmentFiles(
        [makeFile('a.pdf', 'application/pdf', 1000)],
        { householdStorageUsed: 0, maxHouseholdBytes: Number.NaN },
      ),
    ).toThrow(TypeError)
    expect(() =>
      validateAttachmentFiles(
        [makeFile('a.pdf', 'application/pdf', 1000)],
        { householdStorageUsed: 0, maxHouseholdBytes: Number.POSITIVE_INFINITY },
      ),
    ).toThrow(TypeError)
  })
  ...
  it('rejects all new files when existingCount already equals MAX_FILES_PER_EXPENSE', () => {
    const { accepted, rejection } = validateExpenseAttachments(
      ...
      { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: 0, existingCount: MAX_FILES_PER_EXPENSE },
    )
    expect(accepted).toHaveLength(0)
  ...
  it('rejects MIME type with different casing (server regex is case-sensitive)', () => {
    ...
    const { rejection } = validateAttachmentFiles([f], { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: 0 })
    expect(rejection?.code).toBe('unsupportedType')
  })
  ...
  it('handles empty input list without error', () => {
    const { accepted, rejection } = validateAttachmentFiles([], { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: 0 })
  ...
  it('reports the most recent rejection when multiple violations are present', () => {
    ...
    const { accepted, rejection } = validateExpenseAttachments(
      ...
      { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: 0, existingCount: MAX_FILES_PER_EXPENSE }, // no slots left
    )
    expect(accepted).toEqual([])
    expect(rejection?.code).toBe('maxFilesPerExpense')
  })
  ...
  it('stagedFiles contribute to count AND quota, not just dedupe', () => {
    ...
    const { accepted, rejection } = validateExpenseAttachments(
      ...
      { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: 0, existingCount: 1, stagedFiles: staged },
    )
    expect(accepted).toHaveLength(0)
    expect(rejection?.code).toBe('maxFilesPerExpense')
  })
...
describe('validateExpenseAttachments wrapper', () => {
  it('applies MAX_FILES_PER_EXPENSE cap without needing an explicit maxFiles arg', () => {
    ...
    const { accepted, rejection } = validateExpenseAttachments(input, { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: 0 })
...
describe('validateDocumentFiles wrapper', () => {
  it('does NOT enforce a file count cap', () => {
    ...
    const { accepted, rejection } = validateDocumentFiles(input, { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: 0 })
  ...
  it('still enforces per-file size and household quota (shared invariants)', () => {
    ...
    const { rejection } = validateDocumentFiles([oversized], { maxHouseholdBytes: MAX_HOUSEHOLD_STORAGE, householdStorageUsed: 0 })
    expect(rejection?.code).toBe('exceedsLimit')
  })
})
